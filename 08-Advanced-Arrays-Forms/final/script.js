...
const generatePassword = function (length) {
  ...
  console.log(
    `Valid password found: ${password} (after ${attemptCount} attempts)`,
  );
  return password;
};
...
const descriptions = eurPrices.map(
  (price, i) =>
    `Item ${i + 1}: ${price}€ = ${(price * exchangeRate).toFixed(2)}$`,
);
...
console.log(
  "In stock & under $500:",
  affordableInStock.map((p) => p.name),
);
...
console.log(
  "By salary:",
  bySalary.map((e) => `${e.name}: $${e.salary}`),
);
...
console.log(
  "By dept + salary:",
  byDeptAndSalary.map((e) => `${e.department} — ${e.name}: $${e.salary}`),
);
...
// Finding the minimum
const min = values.reduce((acc, cur) => (cur < acc ? cur : acc), values[0]);
...
const mostExpensive = cart.reduce((acc, cur) =>
...
console.log(
  `Summary: ${summary.totalItems} items, $${summary.totalPrice.toFixed(2)}`,
);
